...
class guideXmlMode:
    """Class for special handling of GuideXML document types."""
    
    def __init__(self):
    	self.lists = [ 'dl', 'ol', 'path', 'table', 'tr', 'ul' ]
	self.objects = [ 'b', 'c', 'comment', 'const', 'i', 'ident', 'keyword', 'path', 'stmt', 'var' ]
    def getIgnoredTags(self):
        "Returns array of tags to be ignored."
        return [ ] + self.lists + self.objects
